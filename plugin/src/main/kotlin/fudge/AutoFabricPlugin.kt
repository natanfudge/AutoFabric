...
//        )
    ...
    fun insertEntrypoints() {
        for (sourceSet in getSourceSets()) {
            ...
            val entrypoints = json["entrypoints"] ?:
            HashMap<String,List<String>>().also { json["entrypoints"] = it }

            entrypointsDir.toFile().listFiles()!!.forEach {
                val keys = it.readText().split("\n")
                val value = it.name.replace(FieldMethodSeparator, "::")
                for (key in keys) {
                    if (entrypoints.containsKey(key)) {
                        ...
                        entrypoints[key] = mutableListOf(value)
                    }
                }
            }
